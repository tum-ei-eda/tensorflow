...
void* Init(TfLiteContext* context, const char* buffer, size_t length) {
  ...
  const flexbuffers::Map& m = flexbuffers::GetRoot(buffer_t, length).AsMap(); //What does this mean?
  ...
  //void *raw;
  //context->AllocatePersistentBuffer(Something like this that has to do with raw)
  return data;
}
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  //TfLiteFloatArray* input_for_channel = TfLiteFloatArrayCreate(sample_count) ?
  std::vector<float> input_for_channel(sample_count); // ScratchBuffer
