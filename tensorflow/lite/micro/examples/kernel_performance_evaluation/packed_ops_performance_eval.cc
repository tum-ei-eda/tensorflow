...
// These files were generated by the generate_model.py file in this folder
...
// dequantize the uint8 output (divide by 255 and cast to float)
float output_dquant( uint8_t x )
{
  return 0.00390625f * static_cast<float>(x);
}

// quantize the float input (in [0,1]) by multiplying with 255
uint8_t input_quant( float x)
{
  return static_cast<uint8_t>(x/0.003921568859368563f);
}

// Performs performance analysis for the MNIST model
...
// `arena_used_bytes` can be used to retrieve the optimal size.
  const int tensor_arena_size = 65536 /* exorbitant space */;
...
// The input is a 32 bit floating point value
  ...
  // For both input samples (one should be a zero, one should be a one)
  for (int h = 0; h < 2; ++h) {

    // Extract the reference data
    float ref_data[2] = {label[h][0], label[h][1]};

    // Run multiple invocations for better timing analysis
    ...
    for (int invoke_index = 0; invoke_index < number_of_invocations; invoke_index++) {
      ...
      // Input needs to be set before every invocation, because it is overwritten during invoke
      for (int i = 0; i < 28; i++) {
        for (int j = 0; j < 28; j++) {
      ...
      // Run invocation and check status
      TfLiteStatus invoke_status = interpreter.Invoke();
      TF_LITE_MICRO_EXPECT_EQ(kTfLiteOk, invoke_status);
    }

    // Stop timer and print duration in terminal
...
TF_LITE_MICRO_TEST(MNISTPackedComparison) {
  ...
  // The models are trained on only zeros and ones in the MNIST dataset.
  // The models are trained in the generate_model.py file in this folder
  // The model flatbuffers and testing data are written into the files in the
  //    models subfolder
  // The models uses DepthwiseConv, Conv, MaxPooling and FullyConnected layers

  // We compare the invoke runtime of packed 4,5,6 bit models and
  // an 8bit unpacked model here. Times are printed to the console.
