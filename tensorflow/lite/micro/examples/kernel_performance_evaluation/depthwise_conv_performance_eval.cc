...
static const int NUM_INVOCATIONS = 1000;

static const int kBatches = 4;
static const int kInputSize = 32;
static const int kInputChannels = 6;
...
static const int kFilterSize = 7;
static const int kDepthMultiplier = 2;
static const int kOutputDepth = kInputChannels*kDepthMultiplier;
static const int kFilterElements = 1*kFilterSize*kFilterSize*kOutputDepth;
static const int kFilterShape[] = {4, 1, kFilterSize, kFilterSize, kOutputDepth};
static float kFilterData[kFilterElements];

static const int kBiasShape[] = {4, 1, 1, 1, kOutputDepth};
static const int kBiasElements = kOutputDepth;
static float kBiasData[kOutputDepth];

static const int kStride = 1;
static const int kOutputSize = ((kInputSize - kFilterSize) / kStride) + 1;

const int kOutputShape[] = {4, kBatches, kOutputSize, kOutputSize, kOutputDepth};
const int kOutputElements = kBatches*kOutputSize*kOutputSize*kOutputDepth;
static float kGoldenData[kOutputElements];


void InitDepthwiseConvTestDataQuantized(
    const int _filter_size, const int output_channels,
    const int input_channels, const int batches, const int _input_size,
    float* input_data,
    float* filters_data,
    float* bias_data) {
  const RuntimeShape _input_shape = {batches, _input_size, _input_size, input_channels};
  const RuntimeShape _filter_shape = {1, _filter_size, _filter_size, output_channels};

  // Initialize input
  for (int b = 0; b < batches; b++) {
    for (int in_x = 0; in_x < _input_size; in_x++) {
      for (int in_y = 0; in_y < _input_size; in_y++) {
        for (int in_channel = 0; in_channel < input_channels; in_channel++) {
          input_data[Offset(_input_shape, b, in_x, in_y, in_channel)] = in_x + in_y;
        }
      }
    }
  }
  ...
  for (int out_channel = 0; out_channel < output_channels; out_channel++) {
    ...
    for (int filter_x = 0; filter_x < _filter_size; filter_x++) {
      for (int filter_y = 0; filter_y < _filter_size; filter_y++) {
        filters_data[Offset(_filter_shape, 0, filter_y, filter_x, out_channel)] = filter_x + filter_y;
      }
    }
  }
}
    ...
    for (int out_y = 0; out_y < output_size; ++out_y) {
      for (int out_x = 0; out_x < output_size; ++out_x) {
        for (int ic = 0; ic < input_channels; ++ic) {
          for (int m = 0; m < kDepthMultiplier; m++) {
            const int oc = m + ic * kDepthMultiplier;
...
TF_LITE_MICRO_TEST(Int8Performance) {
  ...
  tflite::testing::InitDepthwiseConvTestDataQuantized(tflite::testing::kFilterSize, tflite::testing::kOutputDepth,
  ...
  tflite::testing::InitGoldenData(tflite::testing::kFilterSize, tflite::testing::kOutputDepth,
                                  tflite::testing::kInputChannels, tflite::testing::kBatches, tflite::testing::kInputSize,
                                  tflite::testing::kInputData,
                                  tflite::testing::kFilterData,
                                  tflite::testing::kBiasData,
                                  tflite::testing::kGoldenData,
                                  tflite::testing::kStride, tflite::testing::kOutputSize, 0);
  ...
  builtin_data.depth_multiplier = tflite::testing::kDepthMultiplier;
  ...
  tflite::testing::TestDepthwiseConvQuantizedPerChannel(
      ...
      tflite::testing::kFilterShape, tflite::testing::kFilterData, filter_quantized, tflite::testing::kBiasShape, tflite::testing::kBiasData,
      bias_quantized, tflite::testing::kOutputShape, tflite::testing::kGoldenData, golden_quantized, output_data,
      output_scale, output_zero_point, builtin_data, tflite::testing::NUM_INVOCATIONS);
}
...
TF_LITE_MICRO_TEST(Uint8Performance) {
  ...
  tflite::testing::TestDepthwiseConvQuantizedPerLayer(
      tflite::testing::kInputShape, tflite::testing::kInputData, input_quantized, input_scale, input_zero_point,
      tflite::testing::kFilterShape, tflite::testing::kFilterData, filter_quantized, filter_scale,
      ...
      output_zero_point, builtin_data, tflite::testing::NUM_INVOCATIONS);
}
...
TF_LITE_MICRO_TEST(Int8PaddingPerformance) {
  ...
  constexpr int output_size = tflite::testing::kInputSize;

  const int output_shape[] = {4, tflite::testing::kBatches, output_size, output_size, tflite::testing::kOutputDepth};
  constexpr int output_elements = tflite::testing::kBatches*output_size*output_size*tflite::testing::kOutputDepth;
  float kGoldenData[output_elements];
  ...
  tflite::testing::InitGoldenData(tflite::testing::kFilterSize, tflite::testing::kOutputDepth,
                                  ...
                                  tflite::testing::kBiasData,
                                  kGoldenData,
                                  tflite::testing::kStride, output_size, (tflite::testing::kFilterSize-1)/2);
...
TF_LITE_MICRO_TEST(Uint8PaddingPerformance) {
  ...
  tflite::testing::TestDepthwiseConvQuantizedPerLayer(
      tflite::testing::kInputShape, tflite::testing::kInputData, input_quantized, input_scale, input_zero_point,
      tflite::testing::kFilterShape, tflite::testing::kFilterData, filter_quantized, filter_scale,
      filter_zero_point, tflite::testing::kBiasShape, tflite::testing::kBiasData, bias_quantized, kGoldenData,
      golden_quantized, output_shape, output_data, output_scale,
      output_zero_point, builtin_data, tflite::testing::NUM_INVOCATIONS);
}
